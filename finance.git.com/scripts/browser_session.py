...
def find_browser(preferred="auto", require_cst=False):
    ...
    for browser in browser_choices(preferred):
        try:
            ...
            has_cst = any(is_cst_url(p.get("url", "")) for p in pages)
...
def validate_auth(token, company_id):
    ok, _ = validate_auth_detail(token, company_id)
    return ok


def validate_auth_detail(token, company_id, attempts=1, interval=1.0):
    if not token or not company_id:
        missing = []
        if not token:
            missing.append("token")
        if not company_id:
            missing.append("companyId")
    ...
    for attempt in range(1, attempts + 1):
        try:
            ...
            if payload is not None:
                if is_ok(payload):
                    return True, f"queryCompany 校验成功（第 {attempt} 次）"
                msg = (
                    payload.get("msg")
                ...
                last_detail = f"queryCompany 返回非成功状态（HTTP {status_code}）：{msg}"
            else:
                body_snippet = response.text.strip().replace("\n", " ")[:200]
                last_detail = (
                    f"queryCompany 返回非 JSON 响应（HTTP {status_code}，Content-Type: {content_type or '-'}）："
                    f"{body_snippet or '-'}"
                )
        except Exception as exc:
            last_detail = f"queryCompany 请求异常：{exc.__class__.__name__}: {exc}"
...
def get_login_form_state(page):
    raw = cdp_eval(
        ...
        (() => {
          ...
          const smsInput = inputs.find(i => (i.placeholder || '').includes('短信'));
...
def submit_login(page, username, password, captcha_code=None, sms_code=LOGIN_SMS_CODE):
    payload = {
        "username": username,
        "password": password,
    ...
    (() => {{
      ...
      const smsInput = inputs
        .find(i => (i.placeholder || '').includes('短信'));
      ...
      if (smsInput && !payload.smsCode) {{
        return JSON.stringify({{ ok: false, reason: 'sms-required' }});
      }}
      setNativeValue(usernameInput, payload.username);
      if (smsInput) {{
        setNativeValue(smsInput, payload.smsCode);
      }}
...
def ensure_login(
    ...
    auth_ok, auth_detail = validate_auth_detail(
        token,
        selected_company_id,
        attempts=3,
        interval=2.0,
    ...
    if not auth_ok:
        raise RuntimeError(f"自动登录完成，但登录态校验未通过：{auth_detail}")
    if not company_matches(
        selected_company_id,
        desired_company_id,
        selected_company_name,
