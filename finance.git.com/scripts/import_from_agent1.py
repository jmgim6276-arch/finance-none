...
def extract_department_role_relations(role_detail):
    relations = {}
    if not isinstance(role_detail, dict):
        return relations

    if normalize_text(role_detail.get("dataType")) != "DEPARTMENT":
        return relations
    ...
    for item in role_detail.get("data") or []:
        ...
        for user in item.get("users") or []:
            ...
            if user_id:
                user_ids.append(user_id)
        relations[dep_id] = {
            "relationId": item.get("roleDepartmentId") or item.get("relationId") or item.get("id"),
            "departmentTitle": normalize_text(item.get("title")),
...
def update_role_relation(role_info, user_ids, company_id, headers, relation_id=None, department_ids=None):
    payload = {
        "roleId": role_info.get("id"),
        "userIds": sorted(set(user_ids)),
        "companyId": company_id,
    ...
    return requests.post(
        f"{BASE_URL}/api/member/role/update/relation",
        headers=headers,
        json=payload,
        timeout=12,
    ).json()
...
def delete_role_department_relation(relation_id, company_id, headers):
    return requests.delete(
        f"{BASE_URL}/api/member/role/delete/department",
        headers=headers,
        params={"id": relation_id, "companyId": company_id},
        timeout=12,
    ).json()
...
def collapse_department_ids_to_leaf_ids(department_ids, dep_index):
    ids = unique_list([dep_id for dep_id in (department_ids or []) if dep_id not in (None, "")])
    if len(ids) <= 1:
        return ids

    id_to_node = (dep_index or {}).get("id_to_node") or {}
    ancestor_ids = set()
    for dep_id in ids:
        seen = set()
        parent_id = (id_to_node.get(dep_id) or {}).get("parentId")
        while parent_id not in (None, "", -1, 0) and parent_id not in seen:
            if parent_id in ids:
                ancestor_ids.add(parent_id)
            seen.add(parent_id)
            parent_id = (id_to_node.get(parent_id) or {}).get("parentId")

    leaf_ids = [dep_id for dep_id in ids if dep_id not in ancestor_ids]
    return leaf_ids or ids[-1:]


def resolve_fee_row_hierarchy(row, df2, state=None):
    state = dict(state or {})

    def raw_value(label):
        col = get_optional_col(df2, label)
        return normalize_text(row.get(col, "")) if col else ""

    raw_p = raw_value("一级费用科目")
    raw_s = raw_value("二级费用科目")
    raw_t3 = raw_value("三级费用科目")
    raw_t4 = raw_value("四级费用科目")
    ...
    if raw_t3:
        ...
        state["三级费用科目"] = ""
        state["四级费用科目"] = ""
    t3 = raw_t3 or state.get("三级费用科目", "")

    if raw_t4:
        state["四级费用科目"] = raw_t4
    elif raw_t3 or raw_s or raw_p:
        state["四级费用科目"] = ""
    t4 = raw_t4 or state.get("四级费用科目", "")
...
def choose_cashier_fallback_user_ids(workflow_name, approval_specs, user_by_id):
    available_user_ids = sorted(
        user_id
        for user_id in user_by_id.keys()
        if user_id not in (None, "")
    )
    if available_user_ids:
        seed_text = normalize_text(workflow_name) or "cashier"
        digest = hashlib.md5(seed_text.encode("utf-8")).hexdigest()
        picked_index = int(digest[:8], 16) % len(available_user_ids)
        return [available_user_ids[picked_index]]

    fallback_user_ids = []
    for spec in approval_specs or []:
        fallback_user_ids.extend(spec.get("userIds") or [])
    fallback_user_ids = unique_list(fallback_user_ids)
    return fallback_user_ids[:1]
...
def build_workflow_json(workflow_name, approval_specs, copy_spec, user_by_id):
    ...
    if copy_spec:
        cashier_target = {
            "userIds": copy_spec.get("userIds") or [],
            "selectedRole": copy_spec.get("selectedRole"),
        }
        ...
        cashier_target = {
            "userIds": choose_cashier_fallback_user_ids(workflow_name, approval_specs, user_by_id),
            "selectedRole": None,
        }

    if cashier_target and (cashier_target.get("userIds") or cashier_target.get("selectedRole")):
        apply_workflow_target_to_selections(
            end_node["COUNTERSIGN"]["SELECTIONS"],
            cashier_target.get("userIds") or [],
            cashier_target.get("selectedRole"),
            user_by_id,
        )
...
def main():
    ...
    for i, row in df1.iterrows():
        ...
        department_ids = collapse_department_ids_to_leaf_ids(department_ids, dep_index)
            ...
            if "已" in msg or "存在" in msg:
                ...
                if existing_user and existing_user.get("id"):
                    ...
                    if is_ok(update_resp):
                        existing_user["departmentIds"] = list(department_ids)
                        report["step1"]["ok"] += 1
                    else:
                        report["step1"]["fail"].append(
                            {
    ...
    if role_col or role_name_col:
        ...
        for i, row in df1.iterrows():
            ...
            for role_entry in collect_row_role_entries(row, df1):
                ...
                for dep_id in department_ids:
                    ...
                    binding["department_user_ids"].setdefault(dep_id, set()).add(user_id)
        ...
        for role_name, binding in role_bindings.items():
            ...
            if role_detail is None:
                ...
                continue
            if role_info.get("dataType") == "DEPARTMENT":
                existing_relations = extract_department_role_relations(role_detail)
                desired_relations = {
                    dep_id: sorted(user_ids)
                    for dep_id, user_ids in binding["department_user_ids"].items()
                    if dep_id and user_ids
                }
                dept_errors = []

                for dep_id, relation in existing_relations.items():
                    if dep_id in desired_relations:
                        desired_user_ids = desired_relations[dep_id]
                        if sorted(relation.get("userIds") or []) == desired_user_ids:
                            continue
                        rel = update_role_relation(
                            role_info,
                            desired_user_ids,
                            company_id,
                            h,
                        ...
                        if not is_ok(rel):
                            dept_errors.append(
                                f"更新部门 {relation.get('departmentTitle') or dep_id} 失败: {rel.get('message')}"
                            )
                    else:
                        relation_id = relation.get("relationId")
                        if relation_id:
                            rel = delete_role_department_relation(relation_id, company_id, h)
                            if not is_ok(rel):
                                dept_errors.append(
                                    f"删除多余部门 {relation.get('departmentTitle') or dep_id} 失败: {rel.get('message')}"
                                )

                for dep_id, desired_user_ids in desired_relations.items():
                    if dep_id in existing_relations:
                        continue
                    ...
                    if not is_ok(rel) and "存在" not in str(rel.get("message", "")):
                        dept_errors.append(f"新增部门 {dep_id} 失败: {rel.get('message')}")
                    ...
                    report["step1_roles"]["fail"].append(
                        {
                            "角色": role_name,
                            "users": binding["users"],
                            "message": "；".join(dept_errors),
                        }
                    )
                    continue
            else:
                existing_user_ids, _ = extract_standard_role_relation_ids(role_detail)
                ...
                if not (is_ok(rel) or "存在" in str(rel.get("message", ""))):
                    report["step1_roles"]["fail"].append(
                        {
                            "角色": role_name,
                            "users": binding["users"],
    ...
    has_level4_col = any("四级费用科目" in str(c) for c in df2.columns)
    ...
    fee_row_state = {}
    for _, row in df2.iterrows():
        fee_values, fee_row_state = resolve_fee_row_hierarchy(row, df2, fee_row_state)
        p = fee_values["一级费用科目"]
        s = fee_values["二级费用科目"]
        t3 = fee_values["三级费用科目"]
        t4 = fee_values["四级费用科目"] if has_level4_col else ""
        doc = normalize_text(row.get(doc_col, ""))
        people = split_values(row.get(get_col(df2, "单据适配人员"), ""))
        if not (p and s):
            continue
